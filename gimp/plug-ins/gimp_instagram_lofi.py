...
#http://www.youtube.com/watch?v=-2IxCKS7IRc
...
def lofi( img, draw ):
	...
	pdb.gimp_item_set_name(lg,"Lofi Effect")

	#copy image
	drawCopy2=pdb.gimp_layer_new_from_drawable(draw,img)
	pdb.gimp_image_insert_layer(img,drawCopy2,lg,0)
	pdb.gimp_item_set_name(drawCopy2,"lofiBG")

	#copy image
	drawCopy=pdb.gimp_layer_new_from_drawable(draw,img)
	pdb.gimp_image_insert_layer(img,drawCopy,lg,0)
	pdb.gimp_item_set_name(drawCopy,"lofiGradMap")

	#set color for gradient map
	grad=pdb.gimp_gradient_new("lofiGradient")
	pdb.gimp_context_set_foreground((38,15,79))
	pdb.gimp_context_set_background((255,110,17))
	...
	pdb.gimp_layer_set_opacity(drawCopy,25)
	pdb.gimp_layer_set_mode(drawCopy,3)

	#adjust brightness contrast
	pdb.gimp_brightness_contrast(drawCopy2, 16, 10)

	#adjust curves colors
	pdb.gimp_curves_spline(drawCopy2, 0, 6, (3,20, 61,61, 255,255) )
	...
	#add vignette
	l = pdb.gimp_layer_new(img,w,h,1,"lofiVignette",100.0,0)
...
register( "gimp_instagram_lofi",
  "Add Instagram lofi effect",
  "Add Instagram lofi effect",
  "Marco Crippa",
  "(©) 2013 Marco Crippa",
  "2013-01-08",
  "<Image>/Filters/Instagram/Lofi",
  'RGB*',
  [],
  '',
  lofi)
